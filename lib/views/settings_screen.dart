...
class _SettingsScreenState extends State<SettingsScreen> with SingleTickerProviderStateMixin {
          ...
          Row(
            children: [
              Container(
                ...
                child: Icon(Icons.settings_suggest_rounded, color: AppColors.primaryNavy, size: 28),
              ),
                ...
                _buildSectionCard(
                  ...
                  child: Column(
                    children: [
                      ...
                      const Divider(height: 32),
                      _buildColorPickerRow(context, 'Admin Sidebar', 'Sidebar background', viewModel.adminSidebarColor, (c) => viewModel.setAdminSidebarColor(_colorToHex(c)), textTheme),
                    ],
                  ),
                ),
              ],
  ...
  void _showColorPickerDialog(BuildContext context, Color initialColor, Function(Color) onColorChanged) {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title: Row(
          children: [
            Icon(Icons.colorize_rounded, color: AppColors.primaryNavy),
  ...
  Widget _buildLogoImage(AppSettingsViewModel viewModel) {
    ...
    return Icon(Icons.add_photo_alternate_outlined, size: 40, color: AppColors.textSecondary);
  }
  ...
  Widget _buildContributorsList(AppSettingsViewModel viewModel, TextTheme textTheme) {
    ...
    return Column(
      children: [
        if (contributors.isEmpty) Text('No team members added.', style: TextStyle(fontStyle: FontStyle.italic, color: AppColors.textSecondary))
        else Wrap(
        ...
        OutlinedButton.icon(onPressed: viewModel.addContributor, icon: const Icon(Icons.add_rounded), label: const Text('Add Team Member'),
          style: OutlinedButton.styleFrom(padding: EdgeInsets.symmetric(horizontal: 24, vertical: 16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)), side: BorderSide(color: AppColors.primaryNavy), foregroundColor: AppColors.primaryNavy)),
      ],
    );
  }
            ...
            Row(
              children: [
                AnimatedScale(
                  ...
                  duration: const Duration(milliseconds: 200),
                  child: CircleAvatar(radius: 14, backgroundColor: AppColors.primaryNavy.withValues(alpha: 0.1), child: Text('${index + 1}', style: TextStyle(color: AppColors.primaryNavy, fontWeight: FontWeight.bold, fontSize: 10))),
                ),
                const Spacer(),
