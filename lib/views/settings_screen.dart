...
class _SettingsScreenState extends State<SettingsScreen> with SingleTickerProviderStateMixin {
        ...
        Row(
          children: [
            Container(
              ...
              decoration: BoxDecoration(color: AppColors.primaryNavy.withOpacity(0.1), borderRadius: BorderRadius.circular(16)),
  ...
  Widget _buildPlatformHeader(BuildContext context, String title, TextTheme textTheme) {
    return Container(
      ...
      decoration: BoxDecoration(color: AppColors.primaryNavy.withOpacity(0.05), borderRadius: BorderRadius.circular(12), border: Border.all(color: AppColors.primaryNavy.withOpacity(0.1))),
    ...
    return Row(
      children: [
        GestureDetector(
          ...
          child: Container(
            width: 50, height: 50,
            decoration: BoxDecoration(color: _hexToColor(hex), borderRadius: BorderRadius.circular(12), border: Border.all(color: AppColors.borderLight, width: 2), boxShadow: [BoxShadow(color: _hexToColor(hex).withOpacity(0.2), blurRadius: 8, offset: const Offset(0, 4))]),
          ),
        ),
        const SizedBox(width: 16),
        Expanded(
  ...
  Widget _buildSectionCard(BuildContext context, {required String title, required String subtitle, required IconData icon, required TextTheme textTheme, required Widget child}) {
    return Container(
      width: double.infinity,
      decoration: BoxDecoration(color: AppColors.cardWhite, borderRadius: BorderRadius.circular(24), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.03), blurRadius: 20, offset: const Offset(0, 10))], border: Border.all(color: AppColors.borderLight.withOpacity(0.5))),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Padding(
            ...
            child: Row(
              children: [
                Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: AppColors.primaryNavy.withOpacity(0.05), borderRadius: BorderRadius.circular(12)), child: Icon(icon, size: 20, color: AppColors.primaryNavy)),
                const SizedBox(width: 16),
      ...
      children: [
        if (contributors.isEmpty) const Text('No team members added.', style: TextStyle(fontStyle: FontStyle.italic, color: AppColors.textSecondary))
            ...
            children: List.generate(contributors.length, (index) {
              ...
              return Container(
                width: isMobile ? double.infinity : (MediaQuery.of(context).size.width / 2) - 80,
                padding: const EdgeInsets.all(16),
                decoration: BoxDecoration(color: AppColors.backgroundLight.withOpacity(0.3), borderRadius: BorderRadius.circular(20), border: Border.all(color: AppColors.borderLight)),
                child: Column(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    Row(
                      children: [
                        CircleAvatar(radius: 14, backgroundColor: AppColors.primaryNavy.withOpacity(0.1), child: Text('${index + 1}', style: const TextStyle(color: AppColors.primaryNavy, fontWeight: FontWeight.bold, fontSize: 10))),
                        const Spacer(),
                    ...
                    Row(
                      children: [
                        Expanded(child: _buildSmallField('Name', contributor.name, textTheme, (val) => viewModel.updateContributor(index, val, contributor.role, contributor.subtitle))),
                        const SizedBox(width: 8),
                        Expanded(child: _buildSmallField('Role', contributor.role, textTheme, (val) => viewModel.updateContributor(index, contributor.name, val, contributor.subtitle))),
                      ],
                    ),
                    const SizedBox(height: 8),
                    _buildSmallField('Details', contributor.subtitle, textTheme, (val) => viewModel.updateContributor(index, contributor.name, contributor.role, val), maxLines: 5),
                  ],
                ),
              );
            }),
      ...
      children: [
        Expanded(
          child: ElevatedButton.icon(
            ...
            style: ElevatedButton.styleFrom(padding: const EdgeInsets.symmetric(vertical: 20), elevation: 4, shadowColor: AppColors.primaryNavy.withOpacity(0.3)),
          ),
        ),
      ],
    );
