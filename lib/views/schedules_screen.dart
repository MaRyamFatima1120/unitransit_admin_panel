...
class _SchedulesScreenState extends State<SchedulesScreen> {
  ...
  // Gender Config — loaded dynamically from RTDB, falling back to defaults if permission is denied
                  ...
                  builder: (context, snapshot) {
                    ...
                    if (snapshot.hasError) {
                      return Padding(
                        ...
                        child: Center(child: Text('Error loading schedules: ${snapshot.error}', style: GoogleFonts.poppins())),
                      );
                    }
                    ...
                    final filteredSchedules = allSchedules.where((schedule) {
                      // Filter out unassigned master templates (TBA or Live) on the schedules screen
                      final isUnassignedTemplate = (schedule.date == null || schedule.date!.isEmpty) &&
                                                   (schedule.operatingDays == null || schedule.operatingDays!.isEmpty) &&
                                                   (schedule.departureTime == null ||
                                                    schedule.departureTime == 'TBA' ||
                                                    schedule.departureTime == 'Live' ||
                                                    schedule.departureTime!.isEmpty);
                      if (isUnassignedTemplate) {
                        return false;
                      }
              ...
              try {
                ...
                if (context.mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text('Schedule deleted successfully!'), backgroundColor: AppColors.primaryNavy),
                  );
                }
              } catch (e) {
                if (context.mounted) {
                  ...
                  child: SingleChildScrollView(
                    child: Column(
                      ...
                      children: [
                        ...
                        // Form Section: Planned Route Selection
                        StreamBuilder<List<BusSchedule>>(
                          stream: service.getBusSchedules(),
                          builder: (context, snapshot) {
                            final allRoutes = snapshot.data ?? [];
                            // Only include master route templates (no date, no operating days, no specific departure time)
                            final Map<String, BusSchedule> uniqueRoutes = {};
                            for (var r in allRoutes) {
                              final hasNoDate = r.date == null || r.date!.isEmpty;
                              final hasNoOperatingDays = r.operatingDays == null || r.operatingDays!.isEmpty;
                              final hasNoTime = r.departureTime == null ||
                                  r.departureTime!.isEmpty ||
                                  r.departureTime == 'TBA' ||
                                  r.departureTime == 'Live';
                              if (hasNoDate && hasNoOperatingDays && hasNoTime) {
                                if (!uniqueRoutes.containsKey(r.route)) {
                                  uniqueRoutes[r.route] = r;
                                }
                              }
                            }
                            final routeNames = uniqueRoutes.keys.toList();
                            
                            return Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text(
                                  'Select Planned Route (Optional)',
                                  style: GoogleFonts.poppins(
                                    fontWeight: FontWeight.bold,
                                    fontSize: 13,
                                    color: AppColors.textDark,
                                ...
                                DropdownButtonFormField<String>(
                                  decoration: InputDecoration(
                                    hintText: 'Select a pre-configured route',
                                    prefixIcon: Icon(Icons.alt_route_rounded, size: 18, color: AppColors.primaryNavy),
                                  ...
                                  items: [
                                    DropdownMenuItem<String>(value: null, child: Text('Custom Route (Manual Entry)', style: GoogleFonts.poppins(fontSize: 13))),
                                    ...routeNames.map((name) => DropdownMenuItem(value: name, child: Text(name, style: GoogleFonts.poppins(fontSize: 13)))),
                                  ],
                                  onChanged: (val) {
                                    if (val != null && uniqueRoutes.containsKey(val)) {
                                      final selectedRoute = uniqueRoutes[val]!;
                                      setDialogState(() {
                                        fromController.text = selectedRoute.from;
                                        toController.text = selectedRoute.to;
                                        // selectedType can be updated if it matches available categories
                                        if (_genderConfigs.keys.contains(selectedRoute.type)) {
                                          selectedType = selectedRoute.type;
                                        }
                              ...
                              _buildTextField(
                                ...
                                onTap: () async {
                                  ...
                                  if (time != null && context.mounted) {
                                    setDialogState(() {
                                      departureTimeController.text = time.format(context);
                                    });
                                  }
                                },
                              )
                        ...
                        Flex(
                          ...
                          children: [
                            ...
                            if (!AppResponsiveUtil.isMobile(context)) const SizedBox(width: 16),
                            if (!AppResponsiveUtil.isMobile(context)) const Expanded(child: SizedBox()),
                          ],
                        ),
                        const SizedBox(height: 24),
      ...
      children: [
        ...
        DropdownButtonFormField<String>(
          ...
          decoration: InputDecoration(
            ...
            prefixIcon: Icon(Icons.directions_bus_outlined, size: 18, color: AppColors.textSecondary),
            contentPadding: const EdgeInsets.symmetric(horizontal: 14, vertical: 12),
            border: OutlineInputBorder(borderRadius: BorderRadius.circular(10)),
            filled: true,
            fillColor: AppColors.backgroundLight.withOpacity(0.5),
