      ...
      child: GestureDetector(
        ...
        onHorizontalDragEnd: (details) {
          if (details.primaryVelocity != null) {
            final viewModel = context.read<DriversViewModel>();
  ...
  void _showAddDriverDialog(BuildContext context, DriversViewModel viewModel) {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
        title: Text('Add New Driver', style: TextStyle(color: AppColors.primaryNavy, fontWeight: FontWeight.bold)),
        content: SizedBox(
          width: 600,
          child: Form(
            key: formKey,
  ...
  Widget _buildDatePicker(BuildContext context, String label, DateTime selectedDate, Function(DateTime) onPick) {
    return InkWell(
      ...
      child: InputDecorator(
        decoration: InputDecoration(
          labelText: label,
          prefixIcon: Icon(Icons.calendar_today, size: 20, color: AppColors.primaryNavy),
          border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)),
          contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
        ),
        child: Text(
      ...
      child: AnimatedContainer(
        ...
        child: Row(
          children: [
            ...
            Expanded(flex: 1, child: Text(d.assignedBus, style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: AppColors.primaryNavy))),
            Expanded(flex: 2, child: driverSchedules.isEmpty ? Text('None', style: TextStyle(fontSize: 12, color: Colors.grey)) : Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: driverSchedules.map((s) => Padding(padding: EdgeInsets.only(bottom: 2.0), child: Tooltip(message: "${s.departureTime ?? 'Live'} • ${s.stops.join(' ➔ ')}", child: Container(padding: EdgeInsets.symmetric(horizontal: 6, vertical: 2), decoration: BoxDecoration(color: AppColors.primaryNavy.withValues(alpha: 0.05), borderRadius: BorderRadius.circular(4)), child: Text(s.route, style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: AppColors.primaryNavy), overflow: TextOverflow.ellipsis, maxLines: 1))))).toList())),
