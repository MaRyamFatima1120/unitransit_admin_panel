...
import 'package:unitransit_admin/views/buses_screen.dart';
import 'package:unitransit_admin/views/audit_logs_screen.dart';
import 'package:unitransit_admin/views/analytics_screen.dart';
import 'package:unitransit_admin/views/app_config_screen.dart';
import 'package:unitransit_admin/views/maintenance_screen.dart';
import 'package:unitransit_admin/views/safety_alerts_screen.dart';
    ...
    return Selector<DashboardViewModel, int>(
      ...
      builder: (context, selectedIndex, _) {
        ...
        return Scaffold(
          ...
          drawer:
              isMobile
                  ? const Drawer(width: 280, child: Sidebar(isCollapsed: false))
                  : null,
          body: Row(
            children: [
              if (!isMobile) Sidebar(isCollapsed: isTablet),
              Expanded(
                child: Column(
                  children: [
                    const Header(),
                          ...
                          children: [
                            if (isSuperAdmin ||
                                [
                                  ...
                                  context.read<LoginViewModel>().subRole,
                                ))
                              _QuickChip(
                                label: 'Add Student',
    ...
    return Selector<DashboardViewModel, Map<String, dynamic>>(
      ...
      builder: (context, stats, _) {
        final cards = [
          ...
          // ── Super Admin Only: Admin Team Card ───────────────────────
          if (isSuperAdmin)
            FadeInSlide(
          ...
          builder: (context, constraints) {
            ...
            final cols = constraints.maxWidth > 1200
                ...
                : (constraints.maxWidth > 900
                    ? (cardCount > 4 ? 4 : cardCount)
                    : (constraints.maxWidth > 650 ? 3 : (constraints.maxWidth > 400 ? 2 : 1)));
            
            // Dynamic aspect ratio to prevent vertical stretching and content overflow
            double aspectRatio;
            if (isMobile) {
              aspectRatio = cols == 1 ? 2.2 : 1.1;
            } else if (cols == 2) {
              aspectRatio = 1.5;
            } else if (cols == 3) {
              aspectRatio = 1.25;
            ...
            children: [
              Row(
                ...
                children: [
                  ...
                  Flexible(child: _buildTrendBadge()),
                ],
              ),
              const SizedBox(height: 12),
              Column(
...
class DashboardChart extends StatelessWidget {
                  ...
                  titlesData: FlTitlesData(
                    ...
                    bottomTitles: AxisTitles(
                      sideTitles: SideTitles(
                        ...
                        reservedSize: 32,
                        getTitlesWidget:
                            (value, meta) => _getBottomTitles(value, scale),
                      ),
                    ),
  ...
  Widget _getBottomTitles(double value, String scale) {
    String text = '';
    if (scale == 'Day' && value.toInt() % 4 == 0)
      text = '${value.toInt()}:00';
    else if (scale == 'Week' && value.toInt() >= 0 && value.toInt() < 7)
      text = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'][value.toInt()];
    else if (scale == 'Month' && value.toInt() % 5 == 0)
      text = 'Day ${value.toInt() + 1}';
    else if (scale == 'Year' && value.toInt() >= 0 && value.toInt() < 12)
      text =
          [
            'Jan',
            'Feb',
...
class _FloatingBusIconState extends State<FloatingBusIcon>
    with SingleTickerProviderStateMixin {
