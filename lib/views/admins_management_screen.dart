...
class _AdminsManagementScreenState extends State<AdminsManagementScreen> {
        ...
        FadeInSlide(
          ...
          child: Column(
            ...
            children: [
              ...
              Text('Manage administrative access, roles and security.',
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(value, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: AppColors.textDark), overflow: TextOverflow.ellipsis),
                Text(label, style: TextStyle(color: AppColors.textSecondary, fontSize: 11), overflow: TextOverflow.ellipsis),
              ],
            ),
          ),
        ],
          ...
          return AlertDialog(
            ...
            title: Row(
              children: [
                Container(
                  ...
                  child: Icon(Icons.person_add_rounded, color: AppColors.primaryNavy, size: 20),
                ),
                const SizedBox(width: 16),
            ...
            content: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  Text(
                    'A new administrative account will be created.',
                    style: TextStyle(color: AppColors.textSecondary, fontSize: 13),
                  ),
                  const SizedBox(height: 24),
            ...
            actions: [
              ...
              ElevatedButton(
                onPressed: _isCreating ? null : () async {
                  ...
                  try {
                    await _createAdmin(context, nameController.text, emailController.text, passwordController.text, 'Admin');
  ...
  Widget _buildDialogField(TextEditingController controller, String label, IconData icon, TextInputAction action, {bool isEmail = false, bool isPassword = false}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(label.toUpperCase(), style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, letterSpacing: 1, color: AppColors.textSecondary)),
        const SizedBox(height: 8),
        TextField(
...
class _AdminRowState extends State<_AdminRow> {
  ...
  Widget build(BuildContext context) {
    ...
    final isMobile = AppResponsiveUtil.isMobile(context);
    final String subRole = widget.admin['subRole'] ?? 'Admin';
    final Color badgeColor;
    if (subRole == 'Fleet Manager') {
      badgeColor = Colors.blue;
    } else if (subRole == 'Support Admin') {
      badgeColor = Colors.orange;
    } else if (subRole == 'System Monitor') {
      badgeColor = Colors.purple;
    } else {
      badgeColor = AppColors.primaryNavy;
    }
          ...
          child: Column(
            ...
            children: [
                        ...
                        Row(
                          children: [
                            ...
                            Container(
                              ...
                              decoration: BoxDecoration(color: badgeColor.withValues(alpha: 0.05), borderRadius: BorderRadius.circular(20)),
                              child: Text(subRole.toUpperCase(), style: TextStyle(color: badgeColor, fontSize: 8, fontWeight: FontWeight.bold, letterSpacing: 0.5)),
                            ),
                          ],
                        ),
                        const SizedBox(height: 2),
                        Text(email, style: TextStyle(color: AppColors.textSecondary, fontSize: 12), overflow: TextOverflow.ellipsis),
                      ],
                    ),
                  ),
                ],
              ...
              Row(
                ...
                children: [
                  Text('Added $dateStr', style: TextStyle(color: AppColors.textSecondary, fontSize: 12)),
                  Row(
                    children: [
                      _ActionIconButton(
    ...
    return MouseRegion(
      ...
      child: AnimatedContainer(
        ...
        child: ListTile(
          ...
          subtitle: Row(
            children: [
              Flexible(child: Text(email, style: TextStyle(color: AppColors.textSecondary, fontSize: 13), overflow: TextOverflow.ellipsis, maxLines: 1)),
              const SizedBox(width: 12),
              Icon(Icons.circle, size: 4, color: AppColors.textSecondary),
              const SizedBox(width: 12),
              Text('Added $dateStr', style: TextStyle(color: AppColors.textSecondary, fontSize: 13)),
            ],
          ),
          trailing: Row(
            mainAxisSize: MainAxisSize.min,
