      ...
      child: GestureDetector(
        ...
        onHorizontalDragEnd: (details) {
          if (details.primaryVelocity != null) {
            final viewModel = context.read<StudentsViewModel>();
            final currentIndex = _tabs.indexOf(viewModel.selectedTab);
            if (details.primaryVelocity! < -300) {
              // swipe left -> next tab
              if (currentIndex < _tabs.length - 1) {
                viewModel.setSelectedTab(_tabs[currentIndex + 1]);
              }
            ...
            children: [
              Padding(
                padding: EdgeInsets.only(
                    top: AppResponsiveUtil.isMobile(context) ? 16 : 32,
                    right: AppResponsiveUtil.isMobile(context) ? 16 : 32,
                    bottom: 16),
                child: Align(
                  alignment: Alignment.centerRight,
                  child: _buildTotalCountBadge(),
                ),
              ),
              Container(
        ...
        return FadeInSlide(
          ...
          child: Container(
            ...
            child: Row(
              mainAxisSize: MainAxisSize.min,
              children: [
                Icon(Icons.people_outline_rounded, size: 16, color: AppColors.primaryNavy),
                  ...
                  CircleAvatar(
                    ...
                    child: student.profileImage.isEmpty
                        ? Text(
                            ...
                            style: TextStyle(color: AppColors.primaryNavy, fontWeight: FontWeight.bold, fontSize: 28),
                          )
                        : null,
                  ),
  ...
  Widget _buildSectionTitle(String title) {
    return Row(
      children: [
        ...
        Text(
          title,
          style: TextStyle(fontSize: 12, fontWeight: FontWeight.w800, color: AppColors.primaryNavy, letterSpacing: 1.5),
        ),
      ],
    );
  }
                  ...
                  Padding(
                    ...
                    child: IntlPhoneField(
                      ...
                      decoration: InputDecoration(
                        ...
                        prefixIcon: Icon(Icons.phone, size: 20, color: AppColors.primaryNavy),
  ...
  Widget build(BuildContext context) {
    ...
    return MouseRegion(
      ...
      child: AnimatedContainer(
        ...
        child: Row(
          children: [
            Expanded(flex: 3, child: Row(children: [AnimatedScale(scale: _isHovered ? 1.1 : 1.0, duration: Duration(milliseconds: 200), child: CircleAvatar(radius: 20, backgroundColor: AppColors.primaryNavy.withValues(alpha: 0.1), backgroundImage: s.profileImage.isNotEmpty ? NetworkImage(s.profileImage) : null, child: s.profileImage.isEmpty ? Text(s.name.isNotEmpty ? s.name[0].toUpperCase() : '?', style: TextStyle(color: AppColors.primaryNavy, fontWeight: FontWeight.bold)) : null)), SizedBox(width: 12), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(s.name, overflow: TextOverflow.ellipsis, style: TextStyle(fontWeight: FontWeight.w700, fontSize: 13, color: _isHovered ? AppColors.primaryNavy : AppColors.textDark)), Text(s.email, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: 11, color: AppColors.textSecondary))]))])),
            Expanded(flex: 2, child: Text(s.studentId, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600))),
            Expanded(flex: 2, child: Text(s.regNo, style: TextStyle(fontSize: 12, color: AppColors.textSecondary))),
            Expanded(flex: 2, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(s.department, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: 12)), Text(s.semester, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: 11, color: AppColors.textSecondary))])),
