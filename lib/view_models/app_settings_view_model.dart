...
class AppSettingsViewModel extends ChangeNotifier {
  ...
  String? _tempAdminSidebarColor; // New
  ...
  String get adminSidebarColor => _tempAdminSidebarColor ?? _appInfo?.adminSidebarColor ?? '#1A237E'; // New
  ...
  void _updateAppColors() {
    try {
      AppColors.primaryNavy = Color(int.parse(adminPrimaryColor.replaceFirst('#', '0xFF')));
      AppColors.accentAmber = Color(int.parse(adminAccentColor.replaceFirst('#', '0xFF')));
      AppColors.backgroundLight = Color(int.parse(adminBackgroundColor.replaceFirst('#', '0xFF')));
      AppColors.cardWhite = Color(int.parse(adminCardColor.replaceFirst('#', '0xFF')));
      AppColors.textDark = Color(int.parse(adminTextPrimaryColor.replaceFirst('#', '0xFF')));
      AppColors.textSecondary = Color(int.parse(adminTextSecondaryColor.replaceFirst('#', '0xFF')));
    } catch (e) {
      debugPrint('Error updating AppColors: $e');
    }
  }
  ...
  void _syncTempColors() {
    ...
    _tempAdminSidebarColor = _appInfo!.adminSidebarColor; // New
  ...
  // Admin Setters
  void setAdminPrimaryColor(String hex) { _tempAdminPrimaryColor = hex; _updateAppColors(); notifyListeners(); }
  void setAdminAccentColor(String hex) { _tempAdminAccentColor = hex; _updateAppColors(); notifyListeners(); }
  void setAdminBackgroundColor(String hex) { _tempAdminBackgroundColor = hex; _updateAppColors(); notifyListeners(); }
  void setAdminCardColor(String hex) { _tempAdminCardColor = hex; _updateAppColors(); notifyListeners(); }
  void setAdminTextPrimaryColor(String hex) { _tempAdminTextPrimaryColor = hex; _updateAppColors(); notifyListeners(); }
  void setAdminTextSecondaryColor(String hex) { _tempAdminTextSecondaryColor = hex; _updateAppColors(); notifyListeners(); }
  void setAdminSidebarColor(String hex) { _tempAdminSidebarColor = hex; _updateAppColors(); notifyListeners(); } // New
  ...
  Future<void> saveAppInfo({
    ...
    try {
      final updatedInfo = AppInfoModel(
        ...
        adminSidebarColor: adminSidebarColor, // New
      ...
      try {
        ...
        if (_appInfo != null) {
          final updatedInfo = AppInfoModel(
            ...
            adminSidebarColor: _appInfo!.adminSidebarColor,
            contributors: _appInfo!.contributors,
          );
          await _firebaseService.updateAppInfo(updatedInfo.toMap());
          _appInfo = updatedInfo;
